...
class Trainer:
    def __init__(self, numClasses=4, timeSample=438, Nu=20, C=22, Nc=20, Nt=1, dropout_rate=0.5,weight_decay=0.0001,learning_rate = 0.0005,optimizer_name="adamw"):
    ...
    def train(self,model_name,num_epochs,avg_epoch_save=100,save_to_db=False,max_train_epoch=-1): 
        ...
        for epoch in range(start_epoch,start_epoch+num_epochs):
            for batch_idx, (data,target) in enumerate(self.train_loader):
                ...
                self.loss.backward()
                self.optimizer.step()
            print(f'Epoch [{epoch+1}/{num_epochs+start_epoch}], Loss: {self.loss.item():.4f}' ,end="")
    ...
    def evaluate(self):
        ...
        print(f" Test Accuracy: {100 * correct / total:.2f}%  ,best: {self.max_acc}")
    ...
    def load_model_weight_by_path(self,file_path):
...
trainer = Trainer(numClasses=4, timeSample=438, Nu=22, C=22, Nc=20, Nt=1, dropout_rate=0.5,learning_rate=0.001,optimizer_name="adamw")
...
trainer.load_model_weight_by_path("LOSO\\LOSO_train\\Nu=22-Nt=1-LR=0.005-Opt=sgd-BZ=16\\epoch=400.pt") # find best
trainer.train("tmp5_LOSO-FT_train",1000000,10000,save_to_db=True)