...
def LOSO():
    tester = Tester(Nu=22,Nt=1)
    tester.load_model_weight("LOSO\\LOSO_train\\Nu=22-Nt=1-LR=0.001-Opt=adamw-BZ=8\\epoch=543.pt")
    test_dataset = Dataloader.MIBCI2aDataset("LOSO-test")
    tester.load_testing_dataset(test_dataset)
    tester.evaluate()

def SD():
    tester = Tester(Nu=44,Nt=1)
    tester.load_model_weight("SD\\SD_train\\Nu=44-Nt=1-LR=0.001-Opt=sgd-BZ=8\\epoch=161.pt")
