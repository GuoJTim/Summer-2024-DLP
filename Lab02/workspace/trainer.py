...
class Trainer:
    ...
    def load_training_dataset(self,train_dataset,batch_size,train_method):
    ...
    def load_testing_dataset(self,test_dataset,batch_size,train_method):
        ...
        for Nt in Nt_s:
            for Nu in Nu_s:
                for lr in learning_rates:
                    for batch_sz in bz_s:
                        trainer = Trainer(numClasses=4, timeSample=438, Nu=Nu, C=22, Nc=20, Nt=Nt, dropout_rate=0.5,learning_rate=lr,optimizer_name=opt)
                        train_dataset = Dataloader.MIBCI2aDataset(method+"-train")
                        test_dataset = Dataloader.MIBCI2aDataset(method+"-test")
                        trainer.load_training_dataset(train_dataset,batch_sz,train_method=method)
                        trainer.load_testing_dataset(test_dataset,batch_sz,train_method=method)
                        trainer.train(method+"_train",1000,10,save_to_db=True,max_train_epoch=1000)
                        #trainer.evaluate()

exit(1)
# FT
trainer = Trainer(numClasses=4, timeSample=438, Nu=22, C=22, Nc=20, Nt=1, dropout_rate=0.5)
train_dataset = Dataloader.MIBCI2aDataset("finetune")
test_dataset = Dataloader.MIBCI2aDataset("LOSO-test")
trainer.load_training_dataset(train_dataset,32,train_method="FT")
trainer.load_testing_dataset(test_dataset,32,train_method="FT")
#trainer.load_model_weight() # find best
trainer.train("LOSO+FT_train",350,10,save_to_db=True)