...
class Dataset_Dance2(torchData):
    def __init__(self, root, transform, mode='val2', video_len=7, partial=1.0):
        ...
        for i in range(data_num):
            self.img_folder.append(sorted(glob(os.path.join(root , f'val2/val2_img/{i}/*')), key=get_key2))
            self.label_folder.append(sorted(glob(os.path.join(root , f'val2/val2_label/{i}/*')), key=get_key2))
...
#        self.Generator            = Generator(input_nc=args.D_out_dim, output_nc=3)
    ...
    def val_dataloader(self):
        ...
        dataset = Dataset_Dance2(root=self.args.DR, transform=transform, video_len=self.val_vi_len)  
