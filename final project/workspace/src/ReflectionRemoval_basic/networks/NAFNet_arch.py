...
class NAFNet_wDetHead(nn.Module):
    ...
    def __init__(self, img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28],
        ...
        # --------------------------- Merge sparse & Img -------------------------------------------------------
        self.intro_Det = nn.Conv2d(in_channels=2, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                              bias=True)
    ...
    def forward(self, inp, spare_ref1,spare_ref2):#spare_ref1,spare_ref2
        B, C, H, W = inp.shape
        inp = self.check_image_size(inp)
        base_inp = inp #[:, :3, :, :]
        x = self.intro(inp)
        
        spare_ref = torch.cat([spare_ref1,spare_ref2], dim=1)
