...
#                                 TESTING
# ===========================================================================
def test(RRNet, RDNet,FDNet, eval_loader, epoch=1,iters=100, max_psnr_val=19, Dname='S', SAVE_test_Results = False):
    RRNet.eval()
    FDNet.eval()
    RDNet.eval()
...
# ===========================================================================
    ...
    for epoch in range(args.START_EPOCH,args.EPOCH):
        ...
        for i, train_data in enumerate(train_loader, 0):
            inputs, label, img_name, isFlare, _ = train_data
            ...
            for j in range(batch_size):
                if isFlare[j].item():  # Convert tensor to a boolean value
                    # print(img_name[j].shape)
                    # print(flare_labels_sparse[j].shape)
                    data_mask = img_name[j]
                    binary_mask = data_mask > 0
                    binary_mask = binary_mask.max(dim=0, keepdim=True)[0].int()

                    flare_labels_sparse[j] = Variable(binary_mask).to(local_rank)  # mask as the ground truth
                    reflection_labels_sparse[j] = Variable(data_sparse[j]).to(local_rank)
                else:
                    
                    flare_labels_sparse[j] = Variable(torch.zeros(size=(1,w,h))).to(local_rank)  # empty blank mask
                    reflection_labels_sparse[j] = Variable(data_sparse[j]).to(local_rank)
            all_labels_sparse = torch.max(flare_labels_sparse,reflection_labels_sparse) # or?
            ...
            if args.cutmix and (r < args.cutmix_prob):
                ...
                reflection_mask = RDNet(inputs)
                flare_mask = FDNet(inputs)
                sparse_out = torch.max(reflection_mask,flare_mask) # or?
                #net_inputs = torch.cat([inputs,  ], dim=1)
                # train_output = NAFNet(inputs, sparse_out.detach())
                train_output = NAFNet(inputs, reflection_mask,flare_mask)
            else:
                reflection_mask = RDNet(inputs)
                flare_mask = FDNet(inputs)
            ...
            input_PSNR = compute_psnr(inputs, train_output)
            trian_PSNR = compute_psnr(train_output, labels)
# ===========================================================================
#                                 RD LOSS
# ===========================================================================
            if args.RDNet_Loss_func.lower() =='char':
                RDNet_L1_Loss = loss_char(reflection_mask, reflection_labels_sparse)
            else:
                RDNet_L1_Loss = losses.sigmoid_mse_loss(reflection_mask, reflection_labels_sparse)
            RDNet_L1_Loss2 = losses.TVLoss(args.gamma_rd)(reflection_mask)
...
# epoch:%d,[%d / %d], [lr: %.7f ],[loss:%.5f,loss1:%.5f,loss2:%.5f,loss3:%.5f, ,avg_loss:%.5f || S1_loss:%.5f,S1_TV_loss:%.5f, avg_S1_loss:%.5f ],[in_PSNR: %.3f, out_PSNR: %.3f],time:%.3f
                info = [
                    ...
                    f"[RRNet_loss:{RRNet_loss.item():.5f},RDNet_loss:{RDNet_loss.item():.5f},FDNet_loss:{FDNet_loss.item():.5f},avg_loss:{training_results['Total_loss'] / iter_nums:.5f} ",
            ...
            if (i + 1) % (print_frequency * 5) == 0 and i > 1:
                if args.SAVE_Inter_Results:
                    ...
                    #            reflection_labels_sparse = torch.empty_like(data_sparse).to(local_rank)
                    #          flare_labels_sparse =  torch.zeros(size=(batch_size,1,w,h)).to(local_rank)
                    save_imgs_for_visualR2(save_path, 
                                        inputs, 
                                        labels, 
                                        train_output, 

                                        reflection_labels_sparse.repeat(1, 3, 1, 1), # reflection mask ground truth
                                        flare_labels_sparse.repeat(1, 3, 1, 1),      # flare mask ground truth
                                        all_labels_sparse.repeat(1, 3, 1, 1),        # all ground truth
                                        
                                        reflection_mask.repeat(1, 3, 1, 1),
                                        flare_mask.repeat(1, 3, 1, 1),
                                        sparse_out.repeat(1, 3, 1, 1))
...
# ===========================================================================
            ...
            max_psnr_val_nature20 = test(RRNet=NAFNet, RDNet =RDNet,FDNet=FDNet,
            ...
            max_psnr_val_real20 = test(RRNet=NAFNet, RDNet =RDNet,FDNet=FDNet,  
                                        ...
                                        # SIRR  
            max_psnr_val_wild55 = test(RRNet=NAFNet, RDNet =RDNet,FDNet=FDNet,  
            ...
            max_psnr_val_postcard199  = test(RRNet=NAFNet, RDNet =RDNet,FDNet=FDNet, 
            ...
            max_psnr_val_soild200 = test(RRNet=NAFNet, RDNet =RDNet,FDNet=FDNet,  
